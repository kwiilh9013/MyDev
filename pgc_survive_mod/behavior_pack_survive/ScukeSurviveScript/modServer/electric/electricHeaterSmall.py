...
class ElectricHeaterSmall(ElectricBase):
	"""小太阳逻辑"""
	def __init__(self, severHandler, blockName, pos, dimension, param={}):
		super(ElectricHeaterSmall, self).__init__(severHandler, blockName, pos, dimension, param)
	...
	def Destroy(self, breaks=False):
		...
		if self._kwTimer:
			...
			self._kwTimer = None
		super(ElectricHeaterSmall, self).Destroy(breaks)
		pass
	...
	def LoadBlockData(self):
		"""加载方块数据"""
		super(ElectricHeaterSmall, self).LoadBlockData()
	...
	def SaveBlockData(self):
		"""保存方块数据"""
		super(ElectricHeaterSmall, self).SaveBlockData()
		blockData = self.mBlockDataComp.GetBlockEntityData(self.mDimension, self.mPos)
		if blockData:
			data = blockData[electricConfig.BlockEntityDataKey]
			if not data:
