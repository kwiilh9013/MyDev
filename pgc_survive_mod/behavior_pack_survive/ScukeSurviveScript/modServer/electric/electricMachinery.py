...
class ElectricMachinery(ElectricBase):
	"""机械工作台逻辑"""
	def __init__(self, severHandler, blockName, pos, dimension, param={}):
		super(ElectricMachinery, self).__init__(severHandler, blockName, pos, dimension, param)
		...
		self._eventFunctions = {
			...
			"set_durability":self.SetItemDurability
		}
	...
	def Destroy(self, breaks=False):
		...
		# 解除占用
		self.SetUseKw(False)
		super(ElectricMachinery, self).Destroy(breaks)
		pass
	...
	# endregion
	# region 设置物品耐久,扣除资源
	def SetItemDurability(self,args):
		playerId,item,slot = args.get("__id__"),args.get("item"),args.get("slot")
		compFactory.CreateItem(playerId).SetItemDurability(serverApi.GetMinecraftEnum().ItemPosType.INVENTORY, slot, serverApiMgr.GetItemMaxDurability(item))
		recipe = GetElectricItemRecipe(item)
		res = serverApiMgr.DeductMultiItemsCount(playerId, args['materials'])
		if res:
			outputItem = recipe.get("output")
			outputTime = outputItem.get("time", 1)
			electricInfo = {
				"stage": "deduct_energy",
				"kw": self._kw,
				"time": outputTime,
