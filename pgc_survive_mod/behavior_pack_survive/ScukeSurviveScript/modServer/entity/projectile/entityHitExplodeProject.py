...
class EntityHitExplodeProject(EntityProjectile):
	"""命中爆炸的抛射物 实体对象"""
	def __init__(self, severHandler, entityId, engineTypeStr=None, param={}):
		super(EntityHitExplodeProject, self).__init__(severHandler, entityId, engineTypeStr, param)
	...
	def Destroy(self):
		...
		engineApiGas.AddTimer(0.05, super(EntityHitExplodeProject, self).Destroy)
		pass
	...
	def ProjectileDoHitEffectEvent(self, args):
		...
		if projectileId == self.mEntityId:
			...
			self.StartExplode()
			# 如果有命中音效
			if self.mCfg.get("hit_sound"):
				info = {
					"stage": "sound",
					"path": self.mCfg.get("hit_sound"),
					"pos": (args.get("x"), args.get("y"), args.get("z")),
				}
				self.mServer.SendMsgToAllClient(eventConfig.EntityEffectEvent, info)
			pass
		pass
