...
import math
import random
from ScukeSurviveScript.ScukeCore.utils.vector import Vector3
"""
...
def InRectangleRange(centerPos, radius, targetPos):
    """判断一个位置是否在一个矩形范围之内"""
    if abs(centerPos[0] - targetPos[0]) <= radius and abs(centerPos[1] - targetPos[1]) <= radius and abs(centerPos[2] - targetPos[2]) <= radius:
        return True
    return False
...
def GetOffsetByRot(rot, length):
    ...
    offset = [0,0.0,0]
    if rot[0]:
        rot0 = math.radians(rot[0])
        offset[1] = -length * math.sin(rot0)
        length = abs(length * math.cos(rot0))
    rot1 = math.radians(rot[1])
    offset[0] = -length * math.sin(rot1)
    offset[2] = length * math.cos(rot1)
    return offset
...
def GetValueFromWeightPool(weightPool, totalWeight):
    """
    从权重池中获取一个值
        weightPool = [ {weight: 1, any: Any, ...}, ... ]
        totalWeight = int, 权重总和
    """
    value = None
    poolLen = len(weightPool)
    if poolLen <= 0:
        return value
    if poolLen == 1:
        return weightPool[0]
    # 按照权重随机怪物
    rdm = random.randint(1, totalWeight)
    curWeight = 0
    for val in weightPool:
        curWeight += val.get('weight', 0)
        if rdm <= curWeight:
            value = val
            break
    return value

def GetTotalWeight(weightPool):
    """
    获取权重池的总权重
        weightPool = [ {weight: 1, any: Any, ...}, ... ]
    """
    return sum(val.get('weight', 0) for val in weightPool)
# endregion
...
def VectorLerpLength(startPos, endPos, t, length=None):
    """向量线性插值，根据长度计算插值，而非比例"""
    if t <= 0:
        return startPos
    v = GetVector(startPos, endPos)
    if length is None:
        length = VectorLength(v)
    if t >= length:
        return endPos
    unitV = VectorNormalize(v)
    return tuple(v1_i + t * v2_i for v1_i, v2_i in zip(startPos, unitV))
