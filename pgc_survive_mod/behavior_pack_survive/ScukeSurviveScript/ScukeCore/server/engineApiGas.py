...
def CalMotionByEndPos(entityId, startPos, endPos, t, g=None, f=0.02):
	if entityId:
...
def ChangePlayerDimension(playerId, dimensionId, pos):
	"""
	将玩家传送至某一维度
	:param str playerId: 玩家id
	:param int dimensionId: 维度，0-overWorld; 1-nether; 2-theEnd
	:param tuple pos: 传送的坐标
	:rtype: bool
	:return: 是否设置成功
	"""
	print("change_player_dimension:", dimensionId, pos, GetEntityDimensionId(playerId) == dimensionId)
	# 同一维度设置两次会导致无法复活
...
def GetTopBlockHeight(pos, dimensionId=0):
	"""
    获取地表方块的高度
		pos = (x, z), 如果pos传3位, 不会报错, 但会影响接下来的代码逻辑
		return long 返回的是long型, 本方法已做转换
	"""
	if len(pos) > 2:
		raise ValueError("pos must be (x, z), not", pos)
	comp = compFactory.CreateBlockInfo(levelId)
	y = comp.GetTopBlockHeight(pos, dimensionId)
	if y is None:
		return None
	return int(y)
...
def SetBlockStates(pos, data, dimensionId=0):
	comp = compFactory.CreateBlockState(levelId)
...
def Hurt(toEid, fromEid, damage, cause, knocked=True, childAttackId=None, customTag=None):
	comp = compFactory.CreateHurt(toEid)
	return comp.Hurt(damage, cause, fromEid, childAttackId, knocked, customTag)
...
def GetExtraData(entityId, key):
	...
	获取实体extraData
		entityId 可传levelId
	"""
	comp = compFactory.CreateExtraData(entityId)
	return comp.GetExtraData(key)

def SetExtraData(entityId, key, value):
	"""
	存储实体extraData
		entityId 可传levelId
	"""
	comp = compFactory.CreateExtraData(entityId)
	return comp.SetExtraData(key, value)
