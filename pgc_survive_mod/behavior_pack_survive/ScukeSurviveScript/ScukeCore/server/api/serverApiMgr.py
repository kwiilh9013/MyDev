...
from ScukeSurviveScript.ScukeCore.common.api import commonApiMgr, itemApi
...
_IsVersion120 = None
def IsGameVersion120():
    """判断游戏版本是否为1.20版本"""
    # 获取1.20才有的物品的名字，如果获取到，则表示是1.20
    global _IsVersion120
    if _IsVersion120 is None:
        itemComp = compFactory.CreateItem(levelId)
        # 下界合金锻造模板
        info = itemComp.GetItemBasicInfo("minecraft:netherite_upgrade_smithing_template")
        _IsVersion120 = info is not None
    return _IsVersion120
# endregion
...
def SpawnProjectile(srcId, engineTypeStr, pos, rot, damage=None, power=None, gravity=None, isDamageOwner=False):
    """
    ...
    if isDamageOwner:
        param["isDamageOwner"] = isDamageOwner
...
def GetNearbyHurtEntityList(entityId, radius):
    """获取附近可造成伤害的实体列表, 仅包含有血量的实体"""
    filters = {
        "any_of": [
            { "test": "has_component", "subject": "other", "operator": "equals", "value": "minecraft:health" },
            { "test": "is_family", "subject": "other", "operator": "equals", "value": "player" },
        ]
    }
    entityList = engineApiGas.GetEntitiesAround(entityId, radius, filters)
    return entityList
# endregion
...
def UpdateCarriedItemDurability(entityId, durability, itemName, aux=None):
    ...
    if item and item.get("newItemName") == itemName:
        if aux is None or item.get("newAuxValue") == aux:
            ...
            if hasDurability <= 0:
                # 清除物品
                itemComp.SpawnItemToPlayerCarried({"newItemName": "minecraft:air", "newAuxValue": 0, "count": 0}, entityId)
            else:
...
def GetRayBlockPos(dimension, rayPos, rayRot, rayDistance=16, ignoreBlocks=[]):
    ...
    """
    pos = None
    blockList = serverApi.getEntitiesOrBlockFromRay(dimension, rayPos, rayRot, rayDistance, False, minecraftEnum.RayFilterType.OnlyBlocks)
    if blockList:
