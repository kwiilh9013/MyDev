...
from ScukeSurviveScript.ScukeCore.common.api import commonApiMgr, itemApi, recipesApi
...
def IsCanSee(entityId, targetId, viewRot, pos=None, rotVector=None):
    """判断是否可以看到目标, viewRot=视野角度的一半，单位度"""
    if rotVector is None:
        rot = engineApiGac.GetRot(entityId)
        rotVector = clientApi.GetDirFromRot(rot)
    if pos is None:
        pos = engineApiGac.GetEntityPos(entityId)
    tpos = engineApiGac.GetEntityPos(targetId)
    # 根据entity的rot，和target的pos，判断entity是否可以看到target
    if tpos:
        return commonApiMgr.IsCanSee(pos, tpos, rotVector, viewRot)
    return False
