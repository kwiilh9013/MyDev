...
def GetHasEnchant(item):
    # 获取物品是否携带原版附魔
    return item is not None and "userData" in item and item["userData"] is not None and "ench" in item["userData"]


# 获取物品悬浮文本信息
def SortItemInfo(itemDict, method):
    enchant = GetHasEnchant(itemDict)
    if itemDict is None: return ""
    name, aux = itemDict["newItemName"], itemDict["newAuxValue"]
    userData = itemDict.get("userData", None)
    info = method.GetItemBasicInfo(name, aux, enchant)["itemName"]
    # 如果是潜影盒，则把潜影盒内部的物品的名称一同输出
    if userData and "Items" in userData:
        info += "\n============"
        for index, value in enumerate(userData["Items"]):
            prefix = "\n§7"
            itemName = value["Name"]["__value__"]
            enchant = "tag" in value and "ench" in value["tag"]
            aux = value["Damage"]["__value__"]
            if "Block" in value: aux = value["Block"]["val"]["__value__"]
            name = method.GetItemBasicInfo(itemName, aux, enchant)["itemName"]
            count = value["Count"]["__value__"]
            bookBool = "minecraft:enchanted_book" == itemName
            if bookBool: name = method.GetItemFormattedHoverText(itemName, aux, False, {"ench": value["tag"]["ench"]})
            info += prefix + "§7" + name + " §7x" + str(count)
            if bookBool: info += "\n"
    else:
        info = method.GetItemFormattedHoverText(name, aux, False, userData)
    return info


# 删除物品的原版附魔数据
def DelItemEnchantData(itemDict):
    enchant = GetHasEnchant(itemDict)
    if not enchant: return itemDict
